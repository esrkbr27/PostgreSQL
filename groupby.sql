...
/*
GROUP BY cümlesi bir SELECT ifadesinde satırları sütunları değerlerine göre
özet olarak gruplamak için kullanılır.
GROUP BY cümleciği her grup basına satır döndürür
GROUP BY genelde AVG(), COUNT(), MAX(), MIN(), VE SUM() gibi aggregate 
fonksiyonalrı ile birlikte kullanılır.
*/
...
--NOT; HANGİ SUTUNLARIN GELMESİNİ İSTİYORSAK SELECTE ONU YAZARIZ
--AS İLE OLUŞTURDUGUM TABLOYA İSİM VERİRİM
select isim, sum (urun_miktari) as toplam_urun from manav 
group by isim  --> sectiğin deger ne ise onu gruplandırır.
...
select urun_adi, min(urun_miktari) as minimum_urun_mik, max (urun_miktari) as maks_urun_miktari
from manav where urun_adi is not null group by urun_adi order by maks_urun_miktari
...
select ulke, count(*) from personel group by ulke -->2. yol
...
select cinsiyet, round(avg(maas)) as ortalama_maas from personel
group by cinsiyet;
...
-- Personeli, ulkelere göre ve (şehirler göre calışan sayısını) sorgulayın.
...
select ulke,cinsiyet,count(*) as calisan_sayisi, round(avg(yas)) as yas_ortalaması from personel
...
select ulke,cinsiyet count(*) as calisan_sayisi, avg(yas) as yas_ortalaması from personel
where maas>30000
...
select ulke,cinsiyet, round(avg(yas)) as ortalama_yas, count(*) as calisan_sayisi from personel
where maas>30000
group by cinsiyet,ulke
